...
class UnitTestsPythonSingleCommit(GeoDiffTests):
    ...
    def test_include_create(self):
        ...
        changeset_inv = (
            tmpdir() + "/py" + "test_include_create" + "/" + "changeset_inv.bin"
        )
        patched = tmpdir() + "/py" + "test_include_create" + "/" + "patched_points.gpkg"

        create_dir("test_include_create")

        # include only points table when creating changeset (lines changes are ignored)
        ...
        # check that now it is same file (for included tables)
        self.geodiff.create_changeset(patched, modified, changeset2)
        check_nchanges(self.geodiff, changeset2, 0)
    ...
    def test_include_apply(self):
        ...
        changeset_inv = (
            tmpdir() + "/py" + "test_include_apply" + "/" + "changeset_inv.bin"
        )
        patched = tmpdir() + "/py" + "test_include_apply" + "/" + "patched_points.gpkg"

        create_dir("test_include_apply")

        # create changeset without filter (captures all 6 changes)
        self.geodiff.create_changeset(base, modified, changeset)
        check_nchanges(self.geodiff, changeset, 6)

        # include only points table when applying changeset
        self.geodiff.set_tables_to_include(["points"])
