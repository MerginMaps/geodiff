...
def basetest(
    ...
    changeset2 = (
        tmpdir() + "/py" + testname + "/" + "changeset_after_apply_" + basename + ".bin"
    )
    changeset_inv = (
        tmpdir() + "/py" + testname + "/" + "changeset_inv" + basename + ".bin"
    )
...
class UnitTestsPythonSingleCommit(GeoDiffTests):
    def test_sqlite_no_gis(self):
        basetest(
            self.geodiff, "pure_sqlite", "base.sqlite", "modified_base.sqlite", 4, False
        )

    def test_geopackage(self):
        basetest(
            self.geodiff, "1_geopackage", "base.gpkg", "modified_1_geom.gpkg", 1, True
        )
